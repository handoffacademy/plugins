...
Verify against the source that owns the rule: Google's current Workspace docs for Gmail, Calendar, Sheets, and Drive; Microsoft's current Graph or Outlook docs for Outlook and Microsoft 365; the vendor's own current docs for Notion, Slack, HubSpot, or any other app; and Composio's current documentation index at `https://docs.composio.dev/llms.txt` for anything routed through the bridge.
...
- The term MCP, and tool identifiers of any shape (`COMPOSIO_SEARCH_TOOLS` and its relatives).
- Action ids, internal parameter names, and raw request or response payloads.
- The names of the skills doing the work. Say "the connection check" and "the cost check", not `automation-connector-discovery` or `automation-composio-cost`.
...
I checked your connected tools and found COMPOSIO_SEARCH_TOOLS, Google Calendar (native), and the rest of the controls on your Composio connection.
...
Where it reads from: your Gmail, through the COMPOSIO_* controls on your connection — Supported
...
Error: {"status":403,"message":"Request had insufficient authentication scopes","connection":"composio","toolkit":"gmail"}
...
1. **Inspect the tool list that is actually visible right now.** If the `automation-connector-discovery` skill is installed, invoke it. If it is not installed, do the equivalent inspection yourself: read your own available tools and identify direct/native connectors, a bridge connection — which appears as its own small handful of controls carrying a `COMPOSIO_` prefix, never as one entry per app — and anything the goal needs that you cannot see. **Which apps that bridge is currently authorized for is not a question the tool list answers.** Ask the connection what it is connected to, and never start an authorization in order to find out: opening that flow is an action on the member's account, and discovery is not a reason to take one. Never run a tool that changes data just to find out whether it works.
...
3. **Check the cost, and treat its answer as a gate.** If any step routes through the bridge, use the `automation-composio-cost` skill and verify the current accounting rule from Composio's own current docs before quoting any number. It returns exactly one of `Fits`, `Does not fit`, or `Unverified`, and **only `Fits` may proceed** — the other two mean moving that source to a native connector, lowering the per-run cap, or lowering the cadence, and a fresh check afterwards. Direct connectors do not draw on the bridge's allowance.

   **A bridge step also settles what holds this task inside its limits, and it is worth being plain about.** Treat the bridge as whole-app and default-allow, always: approving an app opens reading along with sending, changing, and deleting, and **there is no per-action or category-level control to teach, mention, link to, or rely on — including anything offered as a preview or a beta.** So what a bridge hands a task cannot be narrowed to reading the way a native connector can. Such a task may still be scheduled, on the complete gate and nothing shorter: **the complete task block `automation-architect` emits, passing every row of its audit table** — the numbered per-app read allowlist, the full prohibition list, the untrusted-tool-result rule, the slug / unchecked-figure / tool-result-URL sanitizer, and every other runtime guardrail — **together with a recorded knowing approval for every app the job reads**, given after the whole-app disclosure and on a later explicit yes. **Any shorter description of this gate is shorthand and never substitutes for the audited block:** where a summary says "read-only rules and knowing approval", it is naming that block, not replacing it. **On a bridge route those written rules are the guardrail** — say that to the member in plain words rather than implying a setting is doing it, prefer a native connector wherever one exists, and remember that a job scheduled this way stays at version one permanently and never graduates.
...
**Where any source comes through the Composio connection, one thing happens before this question, and it takes its own turn.** A cadence asked first is a schedule the member agreed to before they knew what it rests on. So disclose, ask, and stop:

```text
Before we pick a time — this one reads through your Composio connection, and it needs
[name every app the job reads, one by one]. Approving each of those opens all of that
app, including sending and deleting. There is no setting that turns
those off, so what keeps this to reading and writing you a private summary is the rules
written into the job itself. That is a real step down from a direct connection, and it
means this one stays a read-and-prepare job for good. Happy to go ahead on that basis?
```

**Name every app the job reads in that message — not "the apps this needs".** A yes to an unnamed set is not a yes to the app nobody said out loud. **Then wait for their answer, and take only an explicit yes given after that.** An earlier yes — to the job, to the plan, to connecting the app — is not this yes, because it predates the disclosure. Record, per app, that they approved knowingly and when. **Anything short of that is pending, and pending blocks the test and the schedule** rather than becoming a caveat on the card.
...
Build this after Q5, once you have verified capability in Step 0 — never before. Show it, let them react, then add the schedule after Q6 and finalize after Q7. **Where any source comes through the bridge, the card carries three states per app, separately, because they are three different facts:** the connection is attached; that app is technically authorized on it; and the member knowingly approved whole-app use for it, after the disclosure and on a later explicit yes. The third is the one that is usually assumed and it is the one route 2 rests on — write it as `knowingly approved [date]` or `approval pending`, never inferred from the second. The card also states the route, that written rules are the weaker guardrail, that a direct connector is preferred where one exists, and that this job stays at version one.
...
Route: [Direct connections only — narrowed to what it reads. / Through your Composio connection — approving an app there opens all of it, so what holds this to reading and preparing your review is the rules written into the job rather than a setting. That is the weaker of the two arrangements[, and a direct connection is preferred wherever one reaches the same source — omit this clause where they have already declined the direct route]. This job stays a read-and-prepare job permanently.]
Per app it reads, where the route is the connection — all three, on one line each:
  [App] — connection attached: yes/no | authorized on it: yes/no | knowingly approved: [date] / pending
...
Approval mode: Automatically approve (Auto) — never Skip all approvals. This run finishes on its own rather than stopping on a prompt with nobody there to answer it. What this task can reach was narrowed before it was scheduled to the sources named above and the one tool holding the destination named above, as far as those sources allow. Where a source comes through a whole-app bridge connection, that narrowing stops at the connection and **the complete written block below is what holds this task to reading and reporting** — the numbered read allowlist, the full prohibitions, the untrusted-result rule, the sanitizer, and the record that every app it reads was knowingly approved after disclosure. That whole set is the guardrail on this route, not a description of one, and no part of it is optional. That destination tool offers more operations than the single write this task is permitted, and those extra operations are governed by the two permission lines below, which hold even where the tool itself would let one through. [This is the version-one block. Graduation steps 1 and 2 keep this line as it stands; steps 3 and 4 replace it per *Supervised Mode and Graduation*.]
...
Route and guardrail: [One of exactly two lines.
Route 1 — every source is a direct connector: "What this task can reach was narrowed to
the sources above and the one tool holding the destination; nothing else is in reach."
Route 2 — any source comes through the whole-app Composio connection: "This task reads
through a connection that opens whole apps, so what holds it inside its lane is the
written rules in this block rather than a narrowing. That is the weaker of the two
arrangements; a direct connector is preferred wherever one reaches the same source [omit this clause where a decline is recorded]. Each
app it reads was approved by the member after they were told approving opens all of it.
This task stays at version one permanently and never graduates."]
...
Expected cost: [the cost check's verdict — `Fits`, with the worst-case billable calls per run and per month behind it. "No additional cost" only where no step routes through a bridge at all.]
```
...
| 23 The route line, and on route 2 both of its conditions | `Route and guardrail:` — **reject the block where the route is 2 and either the complete written guardrail above or a recorded knowing approval for every app it reads is absent. A pending or unverified approval is a stop: the task is not tested and not scheduled until it is recorded.** |
...
The prohibited-actions line and the run rules stay in the pasted task. They are not decoration — they are the instructions that keep the task inside its lane on every future run. **Which of two things it is depends on the route.** On a native connector — route 1, and the preferred one — the written rules are not the enforcement: the narrowing is, so before the task goes live confirm which connected tools it can actually reach, narrow that to the sources it was designed to read plus the tool that owns its private destination, and keep every other tool that can send, change, or delete out of it. **On a whole-app bridge connection — route 2 — that narrowing stops at the connection, and these written rules are the guardrail rather than a description of one.** Route 2 may still be scheduled, on both conditions together: this block carries the complete written guardrail below, and every app it reads was knowingly approved by the member after they were told approving opens all of it. **Route 2 is the weaker of the two and it is permanently version one** — say so on the card rather than leaving it implied, and take route 1 wherever a native connector reaches the same source. Narrow whatever can be narrowed on either route. **The one write this task is permitted is creating or appending its own report in that named destination, and nothing else** — not another page, not another record, not that same tool used for anything but this report. On Claude's scheduled-task settings, the approval mode is then `Automatically approve (Auto)`, never `Skip all approvals`. A scheduled run happens with nobody sitting there, so a task set to ask first stops on its first prompt and produces nothing. **Auto does not decide what the task may do.** It removes the prompts for whatever the task can already reach, which is why the reach is settled first and why Auto goes on only once that narrowing is actually in place. **The destination's own tool will usually expose more operations than that one write, and the narrowing stops where the product stops letting you narrow.** Say what carries the rest rather than implying nothing does: the prohibited-actions line in the task text, and the supervised runs the member reads before this task is trusted alone. Reach is narrowed as far as the product allows, and those two layers cover what it cannot. **Where the reach cannot be narrowed to those reads and that destination, which route this is decides what happens next.** On route 1 that is a failure: the narrowing was supposed to be the enforcement, so Auto is not set and this task is not scheduled there. On route 2 it is the expected state and not a failure — the connection opens whole apps by design — so Auto is set and the task is scheduled **once the complete written block above passes its audit and every app it reads is recorded as knowingly approved**, and not before. **Only where neither route qualifies** — no narrowing and no complete block, or a product that will not run it unattended at all — is Auto withheld. Then say so plainly and give them the version that works: the same task run on demand while they are there, same rules, same limits, same private destination, and somebody present for anything the run stops to ask.
...
5. **Choosing the location starts the checking rather than ending it.** That a hosted run exists says nothing about whether a hosted run can do *this* task. A location is a different execution environment, so once one is selected, everything this design leans on is re-verified inside that location:
   - **The exact read, from there.** The same connector or bridge reachable from that location, returning the same fields. Where the read goes through the Composio bridge, its reach and its cost are checked for that location at this task's cadence rather than carried over from the design conversation.
...
6. **Set the guardrails where they are actually enforced, before the task is created — and which place that is depends on the route.** **Route 1, every source a direct connector:** the settings are the enforcement, so narrow the reach first and turn Auto on only once that narrowing is real. **Route 2, any source through the whole-app bridge:** the reach cannot be narrowed past the connection, so the enforcement is the emitted block itself. Auto goes on there **only once two things are verified**: the complete written guardrail is in the block — numbered read allowlist, full prohibitions, untrusted-result rule, sanitizer, all of it passing the audit — and every app the task reads is recorded as knowingly approved. **Neither is a formality and a missing one is a stop.** Where neither route qualifies, or the product will not run the task unattended at all, it stays the job the member runs while they are there. The settings to check are:
   - **What the task can reach.** This one is settled first, because it is the list everything below works from. Check which connected tools the task is able to use, and for version one narrow that to the sources it reads plus the tool that owns its private destination, where the only write it is permitted is creating or appending its own report in that named destination. Keep every other tool that can send, post, change, or delete out of its reach. That destination tool will usually expose more operations than the one write, and the narrowing stops where the product stops letting you narrow: the prohibited-actions line in the task text and the supervised runs the member reads are what cover the remainder. **Where the product will not let you narrow it that far, which is the normal case for a whole-app bridge connection, that alone does not stop the schedule.** Route 2 covers it: this task may still be created with `Automatically approve (Auto)` provided the complete written guardrail above is in the block and every app it reads is recorded as knowingly approved. Verify both before you create it — a missing one is a stop, not a caveat. Where neither route is available, or the product will not run it unattended at all, it stays the job the member runs while they are there and reads at the end.
   - **Approval mode.** On Claude's scheduled-task settings, set it to `Automatically approve (Auto)`, never `Skip all approvals`, so the run completes on its own instead of stopping on a prompt nobody is there to answer. **That is the setting for version one and for graduation steps 1 and 2**; a task carrying a step 3 or 4 write is built the other way, with the member's review held to that one new action, per the substitution in *Supervised Mode and Graduation*. Auto does not decide what the task may do — it removes the prompts for whatever the task can already reach, which is why the line above comes first. **On route 1 that means Auto waits for the narrowing to be real. On route 2 there is no narrowing to wait for, so it waits for the complete emitted block passing every audit row, plus a recorded knowing approval for every app the task reads, instead** — the two are the same requirement in different places, and neither route gets Auto without its own one satisfied. The member's review still happens where it always has, on the private draft before anything goes out. Confirm the setting with them in one line rather than trusting a default.
