...
- **Quick Plan.** The short route, and the one the Academy's lesson opens with. Eight short exchanges, a map carrying the Academy route for every area a Run module builds, and at most one full card. **It does no documentation lookups at all:** Step 0's instant inspection of the visible tool list is the whole of its checking, so every capability line it writes is a conditional strategy line carrying `Unverified — confirm at office hours`, and the stop rule in the document rules is what governs each one when somebody sits down to build.
- **The full interview.** Nine questions in their fixed order, a live documentation check per capability at the session gate, and the first three projects in the build order worked into full cards.
...
```
   ...
   **No full card is required in Quick Plan**, so Q9's rule about showing the first-slot project worked out in full does not apply in this mode. How much gets written out is the session scope rule in the document rules, and it is not re-decided here.

**Custom areas are rows.** Where an area no module covers is the one they want to build first, that area gets the single custom card the session scope rule allows, and every other custom area stays a row. **More than two custom areas never quietly becomes the full interview:** say what you are seeing, offer the longer sitting, and take their answer. The rest stay rows either way.
