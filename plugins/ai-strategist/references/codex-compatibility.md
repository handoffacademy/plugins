...
Two browser rules live here, and they are not the same rule. Read both before you name a browser anywhere.
...
The process-only rule is platform-independent, and so is the session gate. Every skill in this plugin checks the capabilities it is about to rely on against live documentation inside the current session, before the first recommendation, on every platform. Nothing carries over: not a check from yesterday's session, not a `Verified` label inside a document the member pastes in, and not anything written in a SKILL.md file.
