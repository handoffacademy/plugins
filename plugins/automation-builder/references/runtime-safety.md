...
The block says "the cap named above", "the identity fields named above", and
"the timezone named above" precisely so that it can be pasted into another
recipe without a word changing.
...
- Count every item you open toward the cap, including an item you open and then set aside. Reach items with bounded queries — a date range and a result limit — never by pulling everything and reporting a few of them. The cap limits work done, not rows printed.
