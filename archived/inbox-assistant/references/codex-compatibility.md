...
Before every state-changing connector call in ChatGPT or Codex, apply the complete fail-closed write-policy preflight in this section manually. The scope gate governs native email, calendar, and contacts connectors plus every tool on the owner's Zapier server. If the exact tool name appears as a Zapier tool name in Task Settings for any action control, it stays in scope as a tool on the owner's Zapier server even when its server identity has no Zapier marker. A descriptive, non-UUID server identity and matching capability from an unrelated domain is outside that scope and is allowed only when the server declares no email, calendar, contacts, or Zapier capability and the exact tool is not listed as a Zapier tool in Task Settings. Inventory notes meet that test; email-capable CRM, sales, marketing, support, and messaging vendors do not. Any tool that can send, draft, reply to, receive, route, sync, sequence, campaign, or otherwise affect email stays in scope regardless of the vendor label. An opaque or UUID-named server, or a tool plausibly involving email, calendar, or contacts whose route is unclear, also stays in scope and fails closed.

For in-scope calls, classify the complete effect set and let the strictest effect govern. A native-connector write is denied; a read with no state change is allowed; a Zapier write needs the matching enabled and tested action control plus an off kill switch. Pending-test is denied unless every supervised bootstrap condition is visible: the interactive controls session, the owner's specific yes, the smallest self-owned target, and no earlier live call for that action. Scheduled runs, ordinary chats, retries, and second actions do not qualify. No runtime hook is installed; this manual preflight and the command-level auditor policy remain load-bearing.
