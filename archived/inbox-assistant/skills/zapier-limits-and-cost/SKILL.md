...
**Cost follows the actions that are turned on.** Every action starts off, so a member using the read-only product spends no Zapier tasks when native reads cover the mailbox. Once actions are on, count every successful Zapier MCP tool call and apply the current task rate verified from live Zapier documentation. At the time of plugin 3.7.1, Zapier documents two tasks per successful MCP tool call and zero for failed calls, but live documentation wins. Read which actions are on and what each `Scope` covers from the `## Action controls` section in `Task Settings`. Narrowing a scope or switching an action off through `/inbox-assistant:tune` is the direct cost lever. Widening a scope or re-enabling an action returns to the specific outcome's contextual capability flow.
